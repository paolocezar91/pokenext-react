...
export async function getStaticProps(context: GetStaticPropsContext) {
  ...
  try {
    ...
    const previousAndAfter = await getAllPokemon({
      offset: pokemonData.id - 1 > 0 ? pokemonData.id - 2 : 0,
      limit: 3,
    });
    return {
      props: {
...
export default function PokemonDetails({
  ...
  useEffect(() => {
    ...
    const getPokemonMetadata = async () => {
      ...
      dispatch({
        type: "SET_SPECIES_CHAIN",
        payload: await generateSpeciesEvolutionChain(ec),
      });
    };
  ...
  if (error)
    return (
      <RootLayout title={`404 - ${t("pokedex.notFound")}`}>
        <div className="container p-4">
          <h2 className="w-fit border-b-2 border-solid border-white text-lg inline">
            404 - {t("pokedex.notFound")}
          </h2>
        ...
        <div className=" wrapper flex flex-col md:flex-row mx-auto p-4 bg-background rounded shadow-md h-[-webkit-fill-available]">
          <div className=" thumb flex flex-col h-[-webkit-fill-available] md:items-start mr-0 md:mr-4 self-center md:self-start">
            ...
            <Controls
              ...
              previousAndAfter={previousAndAfter}
            />
          </div>
          <div
            className="
