  ...
  useEffect(() => {
    const getMovesDetails = async () => {
      if (
        ...
        moves[versionGroupActive].moveset[movesetActive] = moves[
          versionGroupActive
        ].moveset[movesetActive].map((move) => {
          return {
            ...move,
            details: details.results.find((d) => d.name === move.move),
          };
        });

        if (movesetActive === "machine") {
          const detailsMachineIds = moves[versionGroupActive].moveset[
            movesetActive
          ].map((move) =>
            Number(
              getIdFromUrlSubstring(
          ...
          const detailsMachines = (
            await pokeApiQuery.getMachinesByIds(detailsMachineIds)
          ).results;
          moves[versionGroupActive].moveset[movesetActive] = moves[
            versionGroupActive
          ].moveset[movesetActive].map((move) => {
            return {
              ...move,
              tmDetails: detailsMachines.find((d) => d.move.name === move.move),
            };
          });
        }
            ...
            >
              ...
              <td className="p-2">
                <Link
                  ...
                  className="hover:bg-(--pokedex-red-dark) p-1 transition-colors"
                >
          ...
          {!!moves &&
            <div className="version-picker">
              <label className=" text-xs flex flex-col">
                ...
                <Select
                  onChange={(event) =>
                    setVersionGroupActive(event.target.value)
                  }
                >
                  <option
          ...
          {versionGroupActive &&
            <div className="moveset-picker">
              <label className="mt-2 md:mt-0 md:ml-2 text-xs flex flex-col">
                ...
                <Select
                  onChange={(event) =>
                    setMovesetActive(event.target.value as MovesetTypes)
                  }
                >
                  <option
                    className="bg-gray-300 text-white"
