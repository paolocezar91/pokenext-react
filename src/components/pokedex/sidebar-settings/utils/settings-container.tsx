...
export function SettingsContainer({
  ...
  const tooltipTableText = !showSettings
    ? t("settings.openTableSettings")
    : t("settings.closeTableSettings");
  const tooltipThumbText = !showSettings
    ? t("settings.openThumbnailSettings")
    : t("settings.closeThumbnailSettings");

  return (
    <AnimatePresence>
      <div ref={settingsRef} className={`settings relative z-2 ${className}`}>
        <Tooltip
          position="right"
