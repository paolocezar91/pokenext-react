    ...
    return (
      <div className="w-fit learned-by-pokemon w-full flex flex-col flex-1 h-0 h-[-webkit-fill-available]">
        <h3 className="w-fit text-lg mb-4">
          {t("type.moves.title", { type: capitilize(type), length: 0 })}
        </h3>
  ...
  const tableBody = sortedMoves.map((move, idx) => {
    ...
    return (
      ...
      >
        <td className="p-1">
          <Link
            className="transition-colors transition-colors hover:bg-(--pokedex-red-dark) p-1"
        ...
        <td className="p-1 text-center">{move.power ?? "-"}</td>
        <td className="p-1 text-center">{move.pp}</td>
        <td className="p-1 text-center">{move.accuracy ?? "-"}</td>
      </tr>
    );
  });
    ...
    <div className="w-fit learned-by-pokemon w-full flex flex-col flex-1 h-0 h-[-webkit-fill-available]">
      <h3 className="w-fit text-lg mb-4">
        {t("type.moves.title", {
          ...
          length: movesByType.results.length,
        })}
      </h3>
      <div className="h-[-webkit-fill-available]">
