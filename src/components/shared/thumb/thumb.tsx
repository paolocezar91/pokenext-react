...
export default function PokemonThumb({
  ...
  if (!pkmn || !settings) return loading;

  const artworkUrl = getArtwork(Number(pkmn.id), settings.artworkUrl as ArtUrl)
    .normal[0];
  ...
  const loaded = (
    <div
      style={
        (pkmn as IPkmn).types
          ? getBackgroundStyle((pkmn as IPkmn).types)
          : { background: "#CCCCC" }
      }
      className={`
    ...
    >
      <div
        className={`pokemon-shadow bg-[rgba(0,0,0,0.2)] ${classes[0]}`}
      ></div>
