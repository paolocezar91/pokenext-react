...
export type SortMapping<T extends string, U> = (a: U, b: U) => Record<T, [number | string, number | string]>;
...
export function sortResources<T extends string, U>(
  ...
  return (a: U, b: U) => {
    ...
    return sortMapping(a,b)[defaultKey as T][0] as number - (sortMapping(a,b)[defaultKey as T][1] as number);
  };
}
...
export function updateSortKeys<T>(prevSorting: SortingDir<T>[], key: T) {
  ...
  } else if (prevSorting[idx].dir === '+') { // if it's there change it to desc
    updated = [...prevSorting];
    updated[idx].dir = '-';
