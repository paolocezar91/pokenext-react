...
import UserMenuContent, { Session } from '@/components/layout/navbar/nav-user-auth/user-menu-content';
...
describe('UserMenuContent', () => {
  ...
  it('matches snapshot when no session', () => {
    const { asFragment } = render(<UserMenuContent {...baseProps} session={undefined as any} />);
  ...
  it('matches snapshot when session exists', () => {
    ...
    const { asFragment } = render(<UserMenuContent {...baseProps} session={session} />);
    expect(asFragment()).toMatchSnapshot();
  });
});
