...
jest.mock('@/components/layout/navbar/nav-user-auth/user-avatar-button', () => ({
	__esModule: true,
	default: ({ image, onClick, onMouseEnter }: any) => (
		<button data-testid="avatar-btn" onClick={onClick} onMouseEnter={onMouseEnter}>{image ? 'Avatar' : 'NoAvatar'}</button>
	),
}));
...
describe('NavUserAuth', () => {
	...
	it('renders signed out state and opens menu on hover', () => {
		...
		expect(screen.getByTestId('user-menu-content')).toHaveTextContent('SignedOut');
	});

	it('renders signed in state and toggles menu on click', () => {
		useSessionMock.mockReturnValue({ data: { user: { image: 'img-url' } }, status: 'authenticated' });
		render(<NavUserAuth />);
		const avatarBtn = screen.getByTestId('avatar-btn');
		expect(screen.getByText('Avatar')).toBeInTheDocument();
		fireEvent.click(avatarBtn);
		expect(screen.getByTestId('user-menu')).toBeInTheDocument();
		expect(screen.getByTestId('user-menu-content')).toHaveTextContent('SignedIn');
	});
